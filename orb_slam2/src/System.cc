    ...
    , load_map(load_map)
{
    ...
    std::cout << std::endl
              << "ORB-SLAM2 Copyright (C) 2014-2016 Raul Mur-Artal, University "
                 "of Zaragoza."
              << std::endl
    ...
    cout << "OpenCV version: " << CV_VERSION << "." << CV_MAJOR_VERSION << "."
...
void System::TrackStereo(const cv::Mat& imLeft,
...
void System::TrackRGBD(const cv::Mat& im,
...
rlim_t
System::GetCurrentCallStackSize()
{
    struct rlimit rlimit;
    int operation_result;
...
void System::SwitchPose(cv::Mat& pose)
{
    mpTracker->SwitchPose(pose);
}
