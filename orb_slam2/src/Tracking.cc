...
namespace ORB_SLAM2
{
    ...
    Tracking::Tracking(System *pSys, ORBVocabulary *pVoc, FrameDrawer *pFrameDrawer,
                       Map *pMap, KeyFrameDatabase *pKFDB, const int sensor, ORBParameters &parameters) : mState(NO_IMAGES_YET), mSensor(sensor), mbOnlyTracking(false), mbVO(false), mpORBVocabulary(pVoc),
    ...
    void Tracking::SwitchPose(cv::Mat &pose)
    {
        if (!mpLocalMapper->SetNotStop(true))
            return;
